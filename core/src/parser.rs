...
pub struct Instructions<'s> {
    commands: Vec<Instruction<'s>>,
}

impl<'s> Instructions<'s> {
    pub fn iter<'a>(&'a self) -> impl Iterator<Item = &'a Instruction<'s>> {
        self.commands.iter()
    }
}
...
impl ScriptParser {
    ...
    pub fn parse<'s>(&self, source: &'s str) -> Result<Instructions<'s>> {
        let mut cmd: Instructions = Default::default();
        ...
        while let Some(token) = next_token.take() {
            ...
            match token {
                Token::SendLine => {
                    ...
                    cmd.commands.push(Instruction::SendLine(text));
                }
                Token::Expect => {
                    let text = self.parse_text(&mut lex, source, None)?;
                    cmd.commands.push(Instruction::Expect(text));
                }
                Token::Regex => {
                    let text = self.parse_text(&mut lex, source, None)?;
                    cmd.commands.push(Instruction::Regex(text));
                }
                Token::SendControl => {
                    let text = self.parse_text(&mut lex, source, None)?;
                    let mut it = text.chars();
                    if let Some(c) = it.next() {
                        cmd.commands.push(Instruction::SendControl(c));
                        if it.next().is_some() {
                ...
                Token::Wait => {
                    ...
                    cmd.commands.push(Instruction::Wait(num));
                }
